...
import { MenuTodayCard } from "@/components/menu/menu-today-card";
...
import { isMenuEntryToday } from "@/lib/week-days";
              ...
              <CardContent className="p-5">
                ...
                {thisWeek.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {thisWeek.map((item) => {
                      const isToday = isMenuEntryToday(item);
                      return (
                        <li
                          key={`${item.menuDate ?? item.dayLabel}-${item.menuText}`}
                          className={cn(
                            "atm-stripe-item rounded-xl px-3 py-2.5",
                            isToday && "ring-2 ring-primary/35 bg-primary/5"
                          )}
                        >
